...
* Advanced Player differs from Master Player in (4) where it moves like an Intermediate Player (with the same heuristic).
"""
...
class AI:
    ...
    # Move with most consecutive seeds (similar to Intermediate Player)
    def maximise_own(self, game, AI_c):
        opponent = 'X' if AI_c == 'O' else 'O'
        possible_moves = game.empty_cells()
        best_move = max(possible_moves, key=lambda move: self.heuristic(game.board_state, AI_c, move[0], move[1]) + self.heuristic(game.board_state, opponent, move[0], move[1]))
        return best_move
